...
(function () {
  ...
  function relief(x, y, out) {
    ...
    var wn = 0.075 + 0.055 * g(y + 0.38, 0.10);
    if (y < 0.26) { v = 0.27 * g(x, wn) * g(y + 0.28, 0.24); d += v; s += v * 1.15; }

    // Brow ridge, dipping over the bridge.
    v = 0.055 * g(y - 0.16, 0.075) * g(x, 0.40) * (0.55 + 0.45 * Math.min(1, Math.abs(x) / 0.22));
    d += v; s += v * 2.6;

    // Eye sockets, each holding a globe.
    v = 0.075 * (g(x - 0.26, 0.14) * g(y, 0.085) + g(x + 0.26, 0.14) * g(y, 0.085));
    d -= v; s += v * 2.4;
    v = 0.048 * (g(x - 0.26, 0.072) * g(y, 0.052) + g(x + 0.26, 0.072) * g(y, 0.052));
    d += v; s += v * 3.4;

    // Cheekbones.
    v = 0.045 * (g(x - 0.40, 0.18) * g(y + 0.22, 0.20) + g(x + 0.40, 0.18) * g(y + 0.22, 0.20));
    d += v; s += v * 1.3;

    // Philtrum, then the lips with their crease.
    v = 0.022 * g(x, 0.045) * g(y + 0.47, 0.055); d -= v; s += v * 2.2;
    v = 0.062 * g(y + 0.60, 0.078) * g(x, 0.22); d += v; s += v * 2.8;
    v = 0.034 * g(y + 0.61, 0.020) * g(x, 0.20); d -= v; s += v * 4.0;

    // Chin.
    v = 0.05 * g(y + 0.88, 0.12) * g(x, 0.18); d += v; s += v * 1.6;
  ...
  ILLUSIONS.push({
    ...
    params: [
      ...
      { id: "relief", nom: "Relief", min: 40, max: 160, val: 100, div: 100 },
      { id: "oreilles", nom: "Oreilles", min: 0, max: 1, val: 1, bool: true }
    ],

    init: function (env, p) {
      var A = 0.68, B = 1.0, C = 0.86;      // half width, height, depth
      var k = p.relief / 100;
      var R = [0, 0];
      forme = new Forme(MAX);
      forme.construire(function (i, o) {
        if (p.oreilles && (i % 40) === 0) {
          // Ears: flattened discs on the sides, set back from the eye line.
          var side = (i % 80) === 0 ? 1 : -1;
          var a = Math.random() * Math.PI * 2;
          var rr = Math.sqrt(Math.random());
          o[0] = side * (A * fuseau(0) * 0.99 + Forme.flou(0.02));
        ...
        var z = C * nz * (0.55 + 0.45 * t) + R[0] * k * av;
        var j = 0.005;
        o[0] = x + Forme.flou(j);
        o[1] = y + Forme.flou(j);
        o[2] = z + Forme.flou(j);
